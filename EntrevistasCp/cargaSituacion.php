...
class flexibleAccess{
  ...
  function flexibleAccess($dbConn = '', $settings = '')
  {
	    ...
	    $this->dbConn = ($dbConn=='')? mssql_connect($this->dbHost, $this->dbUser, $this->dbPass):$dbConn;
		
		if ( !$this->dbConn ) die(mssql_get_last_message());
	    mssql_select_db($this->dbName, $this->dbConn)or die(mssql_get_last_message());
  ...
  function getPasswordStringForDB($password) {
	switch(strtolower($this->passMethod)){
		  ...
		  case 'pwdencrypt':
		    $password = "pwdencrypt('$password')";break;
		  case 'nothing':
		  default:
			$password = "'$password'";
		}
  ...
  */
  function login($uname, $password, $remember = false, $loadUser = true)
  {
		...
		$password = $this->getPasswordStringForDB($password);
// echo $password; die;
		...
		if ( $loadUser )
		{
			$this->userData = mssql_fetch_array($res);
  ...
  */
  ...
  function insertUser($data){
	...
	$username = $data[$this->tbFields['userID']];
    $password = $this->getPasswordStringForDB($data[$this->tbFields['pass']]);
    foreach ($data as $k => $v ) $data[$k] = "'".$this->escape($v)."'";
    $data[$this->tbFields['pass']] = $password;
	//$lastUserID = $this->getLastIdentity($this->dbTable);
	$this->query ("INSERT INTO {$this->dbTable} (".implode(', ', array_keys($data)).") VALUES (".implode(", ", $data).")");
	//$newUserID = $this->getLastIdentity
	//if ($lastUserID < $newUserID) {
	//	return (int) $newUserID;
	//} else {
	//	$this->error('Error inserting new user', __LINE__);
	//	return 0;
	//}
  }
  /*
  ...
  function removeUser($userID){
	if ($this->tienePermiso('admin')) {
		$this->query ("DELETE FROM {$this->dbTable} WHERE userID = $userID");
		return (@mssql_affected_rows() == 1);
	} else {
		$this->error('Only admin users can remove users', __LINE__);
		return false;
	}
  }
	...
	* Devuelve TRUE si lo tiene, FALSE caso contrario
	*/
	function tienePermiso($perm) {
		if ($this->is_loaded()) {
			$grupo = $this->get_property('idGrupo');
			$res = $this->query("SELECT TOP 1 * FROM grupos WHERE idGrupo='$grupo' AND idPermiso='$perm'",__LINE__);
			if ( @mssql_num_rows($res) > 0) {
				return true;
			} 
		}
	...
	/**
	* Obtiene los nombres de los grupos existentes en el sistema
	*/
	function getAvailableGroups() {
		$res = $this->query("SELECT idGrupo FROM grupos GROUP BY idGrupo",__LINE__);
	...
	/**
	* Obtiene los usuarios disponibles en el sistema
	*/
	function getAvailableUsers() {
		$res = $this->query("SELECT userID, username FROM usuarios",__LINE__);
		if (mssql_num_rows($res)) {
			while ($row = mssql_fetch_array($res)) {
				$result[] = $row;
			}
	...
	/**
	 * Obtiene los usuarios de la base de datos titular disponibles en el sistema
	 */
	function getAvailableUsersT() {
		$res = $this->query("SELECT Numerousuario, Usuario FROM titular.dbo.usuarios",__LINE__);
		if (mssql_num_rows($res)) {
			while ($row = mssql_fetch_array($res)) {
				$result[] = $row;
			}
  ...
  */
  ...
  {
    //echo '<b>Query to execute: </b>'.$sql.'<br /><b>Line: </b>'.$line.'<br />'; die;
	$res = mssql_query($sql, $this->dbConn);
	if ( !$res )
		$this->error(mssql_get_last_message(), $line);
	return $res;
  }
  ...
  */
  ...
  {
	$res = $this->query("SELECT TOP 1 * FROM {$this->dbTable} WHERE {$this->tbFields['userID']} = '".$this->escape($userID)."'");
    if ( mssql_num_rows($res) == 0 )
    	return false;
    $this->userData = mssql_fetch_array($res);
    $this->userID = $userID;
  ...
  */  
  function escape($str) {
    ...
    $str = $this->mssql_real_escape_string($str, $this->dbConn);
    return $str;
  }
  ...
  */  
  function error($error, $line = '', $die = false) {
	...
	function getLastIdentity($dbTable) {
		$res = $this->query ("SELECT IDENT_CURRENT (userID)FROM {$this->dbTable}");
